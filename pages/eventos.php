...
// Obtener eventos
$eventos = $conn->query("SELECT id, bride_name, groom_name, date_event, location, status FROM eventos ORDER BY date_event DESC");
?>
...
<div class="card p-4">
  ...
  <table class="table table-striped">
    ...
    <tbody>
      ...
      <tr>
        <td><?php echo esc($evento['id']); ?></td>
        <td><?php echo esc($evento['bride_name']); ?></td>
        <td><?php echo esc($evento['groom_name']); ?></td>
        <td><?php echo esc($evento['date_event']); ?></td>
        <td><?php echo esc($evento['location']); ?></td>
        <td><?php echo esc($evento['status']); ?></td>
        <td>
          <a href="?page=eventos&action=delete&id=<?php echo $evento['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('¿Eliminar este evento?')">Eliminar</a>
        </td>
      </tr>
      <?php endwhile; ?>
    </tbody>
