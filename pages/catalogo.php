...
// Obtener productos
$productos = $conn->query("SELECT id, code, name, category, price FROM products ORDER BY name ASC");
?>
...
<div class="row g-3">
  ...
  <div class="col-md-4">
    <div class="card p-3 text-center">
      ...
      <div class="card-body">
        <h6 class="card-title"><?php echo esc($prod['name']); ?></h6>
        <p class="card-text"><?php echo esc($prod['category']); ?> - $<?php echo esc($prod['price']); ?></p>
        <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modal<?php echo $prod['id']; ?>">Ver</button>
      </div>
    </div>
  </div>

  <!-- Modal para detalles -->
  <div class="modal fade" id="modal<?php echo $prod['id']; ?>" tabindex="-1">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title"><?php echo esc($prod['name']); ?></h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
          <p><strong>Código:</strong> <?php echo esc($prod['code']); ?></p>
          <p><strong>Categoría:</strong> <?php echo esc($prod['category']); ?></p>
          <p><strong>Precio:</strong> $<?php echo esc($prod['price']); ?></p>
        </div>
      </div>
    </div>
  </div>
