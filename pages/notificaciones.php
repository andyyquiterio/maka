...
$notificaciones = $conn->query("SELECT id, title, message, created_at FROM notifications ORDER BY created_at DESC LIMIT 50");
?>
...
<div class="card p-4">
  ...
  <div class="list-group">
    ...
    <div class="list-group-item d-flex justify-content-between align-items-start">
      <div>
        <h6 class="mb-1"><?php echo esc($notif['title']); ?></h6>
        <p class="mb-1"><?php echo esc($notif['message']); ?></p>
        <small class="text-muted"><?php echo esc($notif['created_at']); ?></small>
      </div>
      <a href="?page=notificaciones&action=delete&id=<?php echo $notif['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('¿Eliminar esta notificación?')">Eliminar</a>
    </div>
    <?php endwhile; ?>
  </div>
</div>
